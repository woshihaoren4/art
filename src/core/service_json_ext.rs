...
pub trait JsonServiceExt<
    In: Serialize + DeserializeOwned + Default + Send + 'static,
...
where
    F: Fn(Ctx, In, ServiceEntity) -> Fut + Send + Sync + 'static,
...
mod test {
    ...
    async fn test_json_service_ext() {
        let _map = MapServiceLoader::default().register_service(
            "",
            JsonService::new(|_c: Ctx, _i: Value, _se: ServiceEntity| async {
                Ok(Value::default())
            }),
        );
    }

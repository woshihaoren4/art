...
impl Display for SelectCfg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", serde_json::to_string(&self).unwrap_or(String::new()))
    }
}
impl SelectCond {
    pub fn number_value_compare(left:Value, right:Value,opt:SelectCond)->anyhow::Result<bool> {
    ...
    pub fn calc(self,mut args:Vec<Value>)->anyhow::Result<bool>{
        match self {
            ...
            _=>{
                return anyhow::anyhow!("SelectCond[{self:?}] no support calc").err()
            }
        }
    }
}
impl SelectTree {
    pub fn get_value(self)->anyhow::Result<Value>{
        match self {
            ...
            SelectTree::Cond(_cond) => {
                anyhow::anyhow!("[SelectTree] this is not value").err()
            }
        }
    }
